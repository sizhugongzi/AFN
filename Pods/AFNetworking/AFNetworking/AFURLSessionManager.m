...
- (instancetype)initWithSessionConfiguration:(NSURLSessionConfiguration *)configuration {
    ...
    //调用completionHandler处理未完成的数据，上传和下载任务。
    //查询是否存在缓存的任务,如果存在,放到字典里
    //（9）最后一大坨代码是为已经存在的task设置delegate（也很重要）
...
//（2）mutableTaskDelegatesKeyedByTaskIdentifier是在初始化方法中创建的字典，它的用处是用来存储并管理每一个NSURLSessionTask对象，字典的value就是NSURLSessionTask对象，key则是task的taskIdentifier。
...
//（2）调用addDelegateForDataTask:uploadProgress:downloadProgress:completionHandler:函数，给刚才创建的NSURLSessionDataTask对象设置代理
//（准确点说应该是将AFURLSessionManagerTaskDelegate对象与NSURLSessionTask对象相关联，并没有设置代理）。
...
//（2）如果AFURLSessionManagerTaskDelegate对象存在，就把任务交给AFURLSessionManagerTaskDelegate对象来完成，任务完成后，由AFURLSessionManagerTaskDelegate对象调用completionHandler、uploadProgressBlock、downloadProgressBlock等block回调，然后把这个task删除。
//（3）调用AFN自身定义的block回调函数
//在AFURLSessionManagerTaskDelegate对象中，如何处理任务？
//AFURLSessionManagerTaskDelegate对象也实现了NSURLSessionDataTask的URLSession:task:didCompleteWithError:代理方法，实现如下：
